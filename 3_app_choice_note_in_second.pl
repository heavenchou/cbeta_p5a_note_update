...
my $SourcePath = "c:/cbwork/xml-p5a/Y/";			# 初始目錄, 最後不用加斜線 /
my $OutputPath = "c:/temp/xml-p5a-new/Y/";		# 目地初始目錄, 如果有需要的話. 最後不用加斜線 /

my $log_file = "3_app_choice_note_in_second_log.txt";		# log 檔 , 記錄 note in note
...
sub tag_app
{
	...
	{
		print LOG "$lb_num : app,choice,note in second\n";
		print LOG "$text\n\n";
		$note_in_note = 0;
	}
    return $text;
...
sub tag_note
{
	...
	if($note_type ne "inline" && $note_type ne "cf")	# inline 這類的不列入統計
	{
		$note_level++;
	}
